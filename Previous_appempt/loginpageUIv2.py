...
class LoginPage(tk.Tk):
    def __init__(self):
        ...
        # Create a frame for the login form without a background and relative position
        # Removed style for a transparent frame
        ...
        PlaceholderEntry(self.login_frame, "Username", textvariable=self.username_var).pack(pady=10, padx=20, fill='x')
        PlaceholderEntry(self.login_frame, "Password", textvariable=self.password_var, show="*").pack(pady=10, padx=20, fill='x')
        
        ttk.Button(self.login_frame, text="Delete all chats", command=self.delete_chats).pack(pady=20)
